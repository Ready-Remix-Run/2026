...
3. Choose a palette (**Crayola 24** for more color accuracy, or **Crayola 16** if that's the box you have), your block size (how many pixels wide × tall each student's mini-page covers), and an encoding:
   ...
   - **RGB Decimal** — the reverse exercise: three plain decimal numbers per square (e.g. `255`, `162`, `0`), and participants convert each one to binary themselves instead of decoding it. The reference chart shows each color's binary triple to check their work against.
   - **Run Length** — no grid at all. Each mini-page instead gets an ordered list of `count x color id` (e.g. `11 x 12`, `1 x 0`, `3 x 12`, ...): color that many cells before moving to the next value, reading left-to-right and wrapping to the next row when one fills up. The reference chart stays decimal-keyed, same as Decimal/Binary. Mirrors real run-length compression, where adjacent same-colored pixels collapse into one entry.
